...
# Function to get the counts for each category
get_chr_counts <- function(chr_name) {
  counts <- c(
    if (sum(chr == chr_name & str_detect(ucsc_ref_gene_group, "TSS")) > 0) paste0("TSS: ", sum(chr == chr_name & str_detect(ucsc_ref_gene_group, "TSS"))),
    if (sum(chr == chr_name & str_detect(ucsc_ref_gene_group, "Body")) > 0) paste0("Body: ", sum(chr == chr_name & str_detect(ucsc_ref_gene_group, "Body"))),
    if (sum(chr == chr_name & str_detect(ucsc_ref_gene_group, "3'UTR")) > 0) paste0("3UTR: ", sum(chr == chr_name & str_detect(ucsc_ref_gene_group, "3'UTR"))),
    if (sum(chr == chr_name & str_detect(ucsc_ref_gene_group, "5'UTR")) > 0) paste0("5UTR: ", sum(chr == chr_name & str_detect(ucsc_ref_gene_group, "5'UTR")))
  )
...
summary <- cor_viz |>
  ...
  summarise(
    ...
    #chr_counts = paste(paste0(chr, " (", table(chr), ")"), collapse = "; "),
    #chr_tss_counts = paste0(unique(chr), " (", sapply(unique(chr), function(x) sum(chr == x & str_detect(ucsc_ref_gene_group, "TSS"))), ")") %>% paste(collapse = "; "),
    #chr_body_counts = paste0(unique(chr), " (", sapply(unique(chr), function(x) sum(chr == x & str_detect(ucsc_ref_gene_group, "Body"))), ")") %>% paste(collapse = "; "),
    #chr_5UTR_counts = paste0(unique(chr), " (", sapply(unique(chr), function(x) sum(chr == x & str_detect(ucsc_ref_gene_group, "5'UTR"))), ")") %>% paste(collapse = "; "),
    #chr_3UTR_counts = paste0(unique(chr), " (", sapply(unique(chr), function(x) sum(chr == x & str_detect(ucsc_ref_gene_group, "3'UTR"))), ")") %>% paste(collapse = "; "),
